...
class account extends AmysqlController
{
	...
	{
		$this -> title = 'Operation log - account - AMH';
	...
	{
		$this -> title = 'Change Password - account - AMH';
		$this -> AmysqlModelBase();
		Functions::CheckLogin();
		...
		if (isset($_POST['submit']))
		{
			...
			if ($status)
			{
				if(empty($new_user_password) || empty($new_user_password2))
					$error = 'New Password and Confirm New Password can not be empty.';
				elseif($new_user_password != $new_user_password2)
					$error = 'New Password and Confirm New Password inconsistent.';
			}
			else
			...
			if (empty($error))
			{
				...
				{
					$this -> status = 'success';
					$this -> notice = 'Change password successfully.';
				}
				else
