...
<table border="0" cellspacing="1"  id="STable" style="width:1000px;">
	...
	{
	?>
		<tr><td colspan="9" style="padding:10px;">暫無數據備份</td></tr>
	<?php	
	}
	else
	{
	...
	?>
			...
			<td>全面備份 <?php echo ($val['backup_options'] == 'N') ? '<br /><i>(无MySQL数据)</i>' : ($val['backup_options'] == 'n' ? '<br /><i>(无wwwroot数据)</i>' : '');?></th>
			<td><?php echo !empty($val['backup_comment']) ? $val['backup_comment'] : '<i>无</i>';?>
			</td>
			<td><?php echo $val['backup_time'];?></td>
			<td>
			<a href="index.php?c=backup&a=backup_revert&revert_id=<?php echo $val['backup_id'];?>" class="button" ><span class="reload icon"></span>一鍵還原</a>
			<a href="index.php?c=backup&a=backup_list&del=<?php echo $val['backup_id'];?>" class="button" onclick="return confirm('确认刪除備份數據:<?php echo $val['backup_file'];?>?');"><span class="cross icon"></span>删除</a>
			</td>
			</tr>
	<?php
		}
