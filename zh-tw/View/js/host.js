...
window.onload = function ()
{
	...
	{
		var v = (this.value == '') ? '主標識域名' : this.value;
		...
		for (var k in php_fpm_arr)
		{
			if (php_fpm_arr[k].name == php_fpm_click_name)
			{
				...
				if(php_fpm_arr[k+1] && parseInt(php_fpm_arr[k].value) > parseInt(php_fpm_arr[k+1].value))
					notice = php_fpm_arr[k].title + '(' + php_fpm_arr[k].value + ')必須小於或等於' + php_fpm_arr[k+1].title + '(' + php_fpm_arr[k+1].value + ')';
					
				if(php_fpm_arr[k-1] && parseInt(php_fpm_arr[k].value) < parseInt(php_fpm_arr[k-1].value))
					notice = php_fpm_arr[k].title + '(' + php_fpm_arr[k].value + ')必須大於或等於' + php_fpm_arr[k-1].title + '(' + php_fpm_arr[k-1].value + ')';
				
				if (parseInt(php_fpm_arr[k].value) == 0)
					notice = php_fpm_arr[k].title + '(' + php_fpm_arr[k].value + ')必須大於0';
