...
class index extends AmysqlController
{
	public $indexs = null;
	public $configs = null;
	public $action_name = array('start' => '啟動' , 'stop' => '停止' , 'reload' => '重載', 'restart' => '重啟');
	public $notice = null;
	...
	function login()
	{
		...
		if (isset($_POST['login']))
		{
			...
			if($login_allow['status'])
			{
				...
				else
				{
					...
					else
					{
						...
						$_POST['password'] = '';
						$this -> LoginError = '賬號或密碼錯誤，登錄失敗。(' . ($login_allow['login_error_sum']+1) . '次)';
			...
			{
			    $this -> LoginError = '登錄出錯已有' . $login_allow['login_error_sum'] . '次。當前禁止登錄，下次允許登錄時間:' . date('Y-m-d H:i:s', $login_allow['allow_time']);
			}
		}
