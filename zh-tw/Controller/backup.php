...
class backup extends AmysqlController
{
	...
	function backup_list()
	{
		...
		if (isset($_GET['del']))
		{
			...
			if (isset($del_info['backup_file']))
			{
				...
				$this -> status = 'success';
				$this -> notice = "刪除備份文件({$file}.amh)執行完成。";
			}
	...
	function backup_remote()
	{
		...
		if (isset($_POST['save']))
		{
			...
			foreach ($input_item as $val)
			{
				if(empty($_POST[$val]))
				{
					$this -> notice = '新增遠程備份配置失敗，請填寫完整數據，*號為必填項。';
					$save = false;
				}
			}
			if($save)
			{
				...
				{
					$this -> status = 'success';
					$this -> notice = 'ID:' . $id . ' 新增遠程備份配置成功。';
					$_POST = array();
				}
				else
					$this -> notice = ' 新增遠程備份配置失敗。';
			}
		}
		...
		if (isset($_GET['del']))
		{
			...
			if(!empty($id))
			{
				...
				{
					$this -> status = 'success';
					$this -> top_notice = 'ID:' . $id . ' 刪除遠程備份配置成功。';
				}
				else
					$this -> top_notice = 'ID:' . $id . '  刪除遠程備份配置失败。';
			}
		}
		...
		if (isset($_POST['save_edit']))
		{
			...
			foreach ($input_item as $val)
			{
				...
				{
					$this -> notice = 'ID:' . $id . ' 編輯遠程備份配置失敗。 *號為必填項。';
					$save = false;
					$this -> edit_remote = true;
				}
			}
			if ($save)
			{
				...
				{
					$this -> status = 'success';
					$this -> notice = 'ID:' . $id . '編輯遠程備份配置成功';
					$_POST = array();
				}
				else
				{
					$this -> notice = 'ID:' . $id . ' 編輯遠程備份配置失败。';
					$this -> edit_remote = true;
				}
			}
	...
	function backup_now()
	{
		...
		if (isset($_POST['backup_now']))
		{
			...
			else
			{
				...
				{
					$this -> status = 'success';
					$this -> notice = $result . ' 已成功創建備份文件。。';
					$_POST = array();
				}
				else
				{
					$this -> status = 'error';
					$this -> notice = $result . ' 備份文件創建失敗' ;
				}
	...
	function backup_revert()
	{
		...
		if (isset($_POST['revert_submit']))
		{
			...
			{
				$this -> status = 'success';
				$this -> notice = $backup_file . ' 數據一鍵還原成功。';
				$_POST = array();
			}
			else
			{
				$this -> status = 'error';
				$this -> notice = $result . $backup_file . ' 一鍵還原失敗。' ;
			}
			$notice = json_encode($this -> notice);
