...
class module extends AmysqlController
{
	...
	function module_list()
	{
		...
		if (isset($_GET['action']) && isset($_GET['name']))
		{
			$name = $_GET['name'];
			$action = $_GET['action'];
			$action_list = array('install' => '安裝' , 'uninstall' => '卸載', 'delete' => '刪除');
			...
			{
				$this -> status = 'error';
				$this -> notice = "$name {$actionName}失敗。";
			}
	...
	function module_down()
	{
		...
		if (isset($_GET['module_name']))
		{
			...
			if (!empty($module_name))
			{
				...
				{
					$this -> status = 'success';
					$this -> notice = "模塊下載成功：$module_name";
				}
				else
				{
				    $this -> status = 'error';
					if (strpos(serialize($status[1]), 'already exist.') !== false)
						$this -> notice = "模塊已存在下載失敗：$module_name";
					else 
						$this -> notice = "模塊下載失敗：$module_name";
				}
			}
			else
			{
