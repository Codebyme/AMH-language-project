...
class mysql extends AmysqlController
{
	...
	function mysql_list()
	{
		...
		if (isset($_GET['del']))
		{
			...
			{
				$this -> status = 'error';
				$this -> notice = "MySQL数据库: {$de_name} 删除失敗。";
			}
		}
		// 清空数据库
		if (isset($_GET['empty']))
		{
			...
			{
				$this -> status = 'error';
				$this -> notice = "MySQL数据库: {$de_name} 清空失敗。";
			}
		}
...
$param_list = array(
array('設置是否開啟InnoDB引擎','InnoDB_Engine', 'On / Off'),
array('是否開啟MySQL二進制日誌','log_bin', 'On / Off'),
array('MyISAM索引緩衝區大小','key_buffer_size', '16M'),
array('客戶端/服務器之間通信緩存區最大值','max_allowed_pa​​cket', '1M'),
array('設置打開表數目最大緩存值','table_open_cache', '64'),
array('設置每一個連接緩存一次性分配的內存','sort_buffer_size', '512K'),
array('TCP/IP和套接字通信緩衝區大小','net_buffer_length', '8K'),
);
		...
		if (isset($_POST['submit']))
		{
			...
			if ($this -> mysqls -> create_database($database_name, $database_character))
			{
				...
				if (isset($_POST['create_user']))
				{
					...
					{
						$this -> status = 'success';
						$this -> notice .= ' 用戶權限创建成功：' . $user_name;
					}
					else
					{
						$this -> status = 'error';
						$this -> notice .= ' 用戶權限创建失敗：' . mysql_error();
					}
				}
				$_POST = array();
			}
			...
			{
				$this -> status = 'error';
				$this -> notice = '数据库创建失敗：' . mysql_error();
			}
		}
	...
	function mysql_password()
	{
		...
		if (isset($_POST['submit']))
		{
			...
			if ($user_action == 'del')
			{
				...
				{
					$this -> status = 'error';
					$this -> notice .= ' MySQL用戶删除失敗： 面板不允许删除root账号。' ;
				}
				elseif ($this -> mysqls -> del_mysql_user($user_name))
				{
					$this -> status = 'success';
					$this -> notice .= ' MySQL用戶删除成功：' . $user_name -> User . ' - ' . $user_name -> Host;
					$_POST = array();
				}
				else
				{
					$this -> status = 'error';
					$this -> notice .= ' MySQL用戶删除失敗：' . mysql_error();
				}
			}
			else
			{
				if ($user_password1 == $user_password2)
				{
					...
					{
						$this -> status = 'error';
						$this -> notice .= ' MySQL用戶密碼修改失敗： 您已開啟面板數據私有保護，面板不可更改root賬號密碼。' ;
					}
					elseif ($this -> mysqls -> set_mysql_password($user_name, $user_password1))
					{
						$this -> status = 'success';
						$this -> notice .= ' MySQL用戶密碼修改成功：' . $user_name -> User . ' - ' . $user_name -> Host;
					...
					{
						$this -> status = 'error';
						$this -> notice .= ' MySQL用戶密碼修改失敗：' . mysql_error();
					}
				}
				else
				{
					$this -> status = 'error';
					$this -> notice = 'MySQL用戶密碼修改失敗：两次密碼不一致。';
				}
			}
		}
