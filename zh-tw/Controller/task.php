...
class task extends AmysqlController
{
	...
	function task_set()
	{
		...
		if (isset($_POST['save_submit']))
		{
			...
			{
				$this -> status = 'success';
				$this -> notice = 'ID' . $id . ' : 編輯保存任務計劃成功。';
				$_POST = array();
			}
			else
				$this -> notice = '編輯保存任務計劃失敗。';
		}
		elseif (isset($_POST['task_submit']))
		{
			...
			{
				$this -> status = 'success';
				$this -> notice = 'ID' . $insert_status . ' : 新增任務計劃成功。';
				$_POST = array();
			}
			else
				$this -> notice = '新增任務計劃失敗。';
		}
		elseif (isset($_GET['del']))
		{
			...
			if($del_status)
				{
					$this -> status = 'success';
					$this -> top_notice = 'ID' . $id . ' : 刪除任務計劃成功。';
				}
				else
					$this -> top_notice = '刪除任務計劃失敗。';
		}
