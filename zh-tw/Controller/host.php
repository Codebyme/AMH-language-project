	...
	function vhost()
	{
		...
		if (isset($_GET['del']))
		{
			...
			if(!empty($del_name))
			{
				...
				{
					$this -> status = 'success';
					$this -> top_notice = $del_name . ' : 删除虛擬主機成功。';
				}
				else
					$this -> top_notice = $del_name . ' : 删除虛擬主機失败。' . implode(',', $result[1]);
			}
		}
		...
		if (isset($_POST['save']))
		{
			...
			else
			{
				...
				if ($result[0])
				{
					...
					$this -> status = 'success';
					$this -> notice = $_POST['host_domain'] . ' : 新增虛擬主機成功。';
					$_POST = array();
				}
				else
					$this -> notice = $_POST['host_domain'] . ' : 新增虛擬主機失败。' . implode(',', $result[1]);
			}
		}
		...
		if (isset($_POST['save_edit']))
		{
			...
			{
				$status = true;
			$top_notice = $host_name . ' : 編輯虛擬主機配置成功。 ';
}
else
{
$this -> status = 'error';
$top_notice = $host_name . ' : 編輯虛擬主機配置失敗。 ' . implode(',', $result[1]);
			}
	...
	{
		$this -> title = 'PHP参数设置 - 虛擬主機 - AMH';
		$this -> AmysqlModelBase();
		Functions::CheckLogin();

$param_list = array(
array('設置PHP時區','date.timezone', 'Asia/Hong_Kong'),
array('是否顯示PHP錯誤信息','display_errors', 'On / Off'),
array('PHP運行內存限制','memory_limit', '68M'),
array('POST數據最大限制','post_max_size', '4M'),
array('上傳文件最大限制','upload_max_filesize', '2M'),
array('上傳文件個數限制','max_file_uploads', '10'),
array('腳本超時時間','max_execution_time', '20'),
array('socket超時時間','default_socket_timeout', '60'),
array('SESSION過期時間','session.cache_expire', '180'),
array('是否開啟短標籤','short_open_tag', 'On / Off'),
array('是否開啟自動字符串轉義','magic_quotes_gpc', 'On / Off'),
array('是否外部字符串自動轉義','magic_quotes_runtime', 'On / Off')
);
