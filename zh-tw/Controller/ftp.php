...
class ftp extends AmysqlController
{
	...
	function ftp_list()
	{
		...
		if (isset($_GET['del']))
		{
			...
			if(!empty($del_name))
			{
				...
				if($get_ftp['ftp_type'] == 'web')
				{
					...
					{
						$this -> status = 'success';
						$this -> top_notice = $del_name . ' : 刪除FTP賬號成功。';
					}
					else
						$this -> top_notice = $del_name . ' : 刪除FTP賬號失敗。' . implode(',', $result[1]);
				}
				else
				    $this -> top_notice = $del_name . ' : ssh FTP賬號web端不可刪除。';
			}
		}
		...
		if (isset($_GET['chown']))
		{
			...
			if(!empty($chown_name))
			{
				...
				if($get_ftp['ftp_type'] == 'web')
				{
					...
					{
						$this -> status = 'success';
						$this -> top_notice = $chown_name . ' : 重寫目錄權限' . $uidname . '成功。';
					}
					else
						$this -> top_notice = $chown_name . ' : 重寫目錄權限' . $uidname . '失敗。';
				}
				else
				    $this -> top_notice = $chown_name . ' : ssh FTP賬號web端不可重寫目錄權限。';
			}
		}
		...
		if (isset($_POST['save']))
		{
			...
			else
			{
				...
				if ($result[0])
				{
					...
					$this -> status = 'success';
					$this -> notice = $_POST['ftp_name'] . ' : 新增FTP賬號成功。';
					$_POST = array();
				}
				else
					$this -> notice = $_POST['ftp_name'] . ' : 新增FTP賬號失敗。' . implode(',', $result[1]);
			}
		}
		...
		if (isset($_GET['edit']))
		{
			...
			{
			     $this -> top_notice = $edit_name . ' : ssh FTP賬號web端不可编辑。';
				 $_POST = array();
			}
		}
		...
		if (isset($_POST['save_edit']))
		{
			...
			if($edit_ftp['ftp_type'] == 'web')
			{
				...
				{
					$status = true;
					$top_notice = $ftp_name . ' : 编辑FTP賬號成功。';
				}
				else
				{
					$this -> status = 'error';
					$top_notice = $ftp_name . ' : 编辑FTP賬號失敗。' . $result[0];
				}
				...
				if (!empty($_POST['ftp_password']))
				{
					...
					{
						$status = true;
						$top_notice .= $ftp_name . ' : 更改FTP密碼成功。';
					}
					else
					{
						$this -> status = 'error';
						$top_notice .= $ftp_name . ' 更改FTP密碼失敗。' . $result[1];
					}
				}
				if(isset($status)) 
					$_POST = array();
